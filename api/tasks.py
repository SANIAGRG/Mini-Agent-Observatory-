...
def call_ollama(prompt: str, bypass_cache: bool = False):
    """One timed call to Ollama. Cache is skipped entirely when bypass_cache
    is True - used by the eval harness so it always tests current behavior
    instead of a saved old answer."""
...
def run_agent_task(prompt: str, bypass_cache: bool = False):
